...
public class BlenderMenu extends AbstractContainerMenu {
    ...
    // 客户端构造函数（由网络包触发）
    public BlenderMenu(int containerId, Inventory playerInv, FriendlyByteBuf extraData) {
        this(containerId, playerInv, new ItemStackHandler(2), new SimpleContainerData(2));
    }

    // 服务端与客户端共用的真正构造函数
    public BlenderMenu(int containerId, Inventory playerInv, IItemHandler itemHandler, ContainerData data) {
        super(ModMenuTypes.BLENDER_MENU.get(), containerId);
        checkContainerDataCount(data, 2);
        this.data = data;

        // 1. 添加机器的专属格子 (SlotItemHandler)
        // 参数: 处理器, 索引, 屏幕X坐标, 屏幕Y坐标
        this.addSlot(new SlotItemHandler(itemHandler, 0, 45, 27));  // 输入1
        this.addSlot(new SlotItemHandler(itemHandler, 1, 115, 27));  // 输入2
